...
const MyMemo = () => {
  //컴포넌트 렌더링시 콘솔에 출력된ㄴ 내역 삭제하기
  React.useEffect(() => console.clear(), []);

  const day = Dayjs();

  //파라미터로 전달되는 단어의 길이를 반환하는 함수
  const getLength = (w) => {
    console.log("getLength()호출됨! :::" + day.format("YY/MM/DD hh:mm:ss.ms"));
    return w.length;
  };
  ...
  /* 
    ...
    콜백의 결과가 저장되는 myLen은 일반 상태값과 동일하게 사용 가능
    -> 즉 myWord가 변경될때만 콜백이 리턴하는 값을 활용하여 myLen을 갱신
    */
